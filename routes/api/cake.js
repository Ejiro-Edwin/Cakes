...
router.put('/:id', (req, res) => {
    ...
    const updateCake = Cake.findByIdAndUpdate(req.params.id,
        {
            name: req.body.name,
            imageUrl: req.body.imageUrl
        }, { new: true }).then(cake => res.json(cake));
    ...
    '/comment/:id', (req, res) => {
        ...
        Cake.findById(req.params.id)
            .then(cake => {
                ...
                cake.comment.unshift(newComment);
...
// @access  Public
router.delete(
    '/comment/:id/:comment_id', (req, res) => {
        Cake.findById(req.params.id)
            .then(cake => {
                // Check to see if comment exists
                if (
                    cake.comment.filter(
                        comments => comments._id.toString() === req.params.comments_id
                    ).length === 0
                ) {
                    return res
                        .status(404)
                ...
                // Get remove index
                const removeIndex = cake.comment
